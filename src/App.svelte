<script>
  ...
  const handleCruiseNight = () => {
		let currentStep = 0;
		let tourPoints = [
			{ label: 'Gus\'s Drive-in', center: [42.783958493217334, -88.41788905454426].reverse(), zoom: 18, bearing: 80, pitch: 60, rotate: false, duration: 0 },
			{ label: 'East Troy Airport', center: [42.79722849966591, -88.37596611461682].reverse(), zoom: 16, bearing: 0, pitch: 0, rotate: true, duration: 60000 },
			{ label: 'Village Square Park', center: [42.78477231354394, -88.40545403606905].reverse(), zoom: 16, bearing: 0, pitch: 0, rotate: true, duration: 10000 },
		]
    ...
    m.setStyle('mapbox://styles/mapbox/satellite-v9');
		...
		tourPoints.map(({ label, center, zoom, bearing, pitch, rotate, duration }) => {
			setTimeout(() => {
				mapComponent?.flyTo({ center: center, zoom: zoom, pitch: pitch, bearing: bearing, speed: 0.3, curve: 1, essential: true });
    ...
    <div class="w-full max-w-3xl mx-auto text-center">
      ...
      >
        ...
        <div class="absolute top-0 left-2 h-[55px] p-2  flex items-center">
          <button class="flex items-center mr-3 text-xs text-white" title="night light" on:click={handleNightLife}>
            <div class="w-6 h-6 mr-1 text-white">
              <FaBeer />
            </div>
            {#if menuIsOpen}
              <span in:fly={{ x: -80, duration: 500 }} out:fly={{ x: -80, duration: 500 }}>Night Life</span>
            {/if}
          </button>
          <button class="flex items-center mr-3 text-xs text-white" title="night light" on:click={handleCruiseNight}>
            <div class="w-6 h-6 mr-1 text-white">
              <IoIosCar />
            </div>
            {#if menuIsOpen}
              <span in:fly={{ x: -80, duration: 500 }} out:fly={{ x: -80, duration: 500 }}>Cruise Night</span>
            {/if}
          </button>
					<button class="flex items-center mr-3 text-xs text-white" title="night light" on:click={handleCruiseNight}>
            <div class="w-6 h-6 mr-1 text-white">
            ...
            {#if menuIsOpen}
              <span in:fly={{ x: -80, duration: 500 }} out:fly={{ x: -80, duration: 500 }}>Take a Tour</span>
            {/if}
          </button>
        </div>
